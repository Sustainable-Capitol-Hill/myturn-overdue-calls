function updateMyTurnSheetsData() {
  ...
  const overdueSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("MyTurn Report: Loans, Overdue Only")
  ...
  const overdueResponse = UrlFetchApp.fetch(
    'https://capitolhill.myturn.com/library/orgLoan/exportLoans',
    {
      ...
      payload: {
        ...
        dueBefore_date: (new Date().getUTCMonth() + 1).toString() + '/' + (new Date().getUTCDate() - 1).toString() + '/' + new Date().getUTCFullYear().toString(),
        dueBefore_time: new Date().getUTCHours() + ':' + new Date().getUTCMinutes().toString(),
        dueBefore_tz: 'UTC',
  ...
  overdueSheet.clear()
  overdueSheet.getRange(1, 1, overdueData.length, overdueData[0].length)
    .setValues(overdueData)

  const checkedOutSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("MyTurn Report: Inventory, List Items, Checked Out")
  ...
  const checkedOutData = Utilities.parseCsv(checkedOutResponse.getContentText())
  checkedOutSheet.clear()
  checkedOutSheet.getRange(1, 1, checkedOutData.length, checkedOutData[0].length)
...
function getAuthenticatedSessionCookie() {
  ...
  const sessionIdCookie = loginResponse.getAllHeaders()['Set-Cookie']
    .split(';')
    .map(function (i) { return i.trim(); })
    // Gosh, not having ES6 support is annoying; I miss `startsWith`
    .filter(function (i) { return i.slice(0, 11) === 'JSESSIONID='; })
    [0]
