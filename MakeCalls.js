...
function insertCallsMadeRow(date, username, outcomeCategory, outcomeNotes) {
  const callsSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Calls Made")
  const callsColumnCount = callsSheet.getMaxColumns()
  callsSheet.insertRowAfter(1)
  callsSheet.getRange(2, 1, 1, callsColumnCount)
    .setValues([[
      date,
      username,
      outcomeCategory,
...
function getAllOverdueUsers() {
  const usersToCallSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Users To Call")
  const usersToCallColumnCount = usersToCallSheet.getMaxColumns()
  const usersToCallHeaders = usersToCallSheet.getRange(1, 1, 1, usersToCallColumnCount).getValues()[0]
  const usersToCallRows = usersToCallSheet.getRange(
    2,
    1,
    usersToCallSheet.getMaxRows() - 1,
    usersToCallColumnCount
  ).getValues().filter(function (i) { return Boolean(i[0]) })
...
function initiateCallDialog(filters) {
  // Set filter defaults
  const minimumDaysOverdue = (filters && typeof filters.minimumDaysOverdue === "number") ? filters.minimumDaysOverdue : 45
  const minimumDaysSinceLastCall = (filters && typeof filters.minimumDaysSinceLastCall === "number") ? filters.minimumDaysSinceLastCall : 14
  const itemNameToSearch = filters && filters.itemNameToSearch

  const usersToCall = getAllOverdueUsers()
    .filter(function (user) {
      return new Date(new Date(user['latest due date']).getTime() + minimumDaysOverdue * 24 * 60 * 60 * 1000) < new Date()
    })
    .filter(function (user) {
      // Has never been called or…
      return !user['most recent call'] ||
        // …hasn't been called in at least the past X days
        // Source: https://stackoverflow.com/questions/7687884/add-10-seconds-to-a-date
        new Date(new Date(user['most recent call']).getTime() + minimumDaysSinceLastCall * 24 * 60 * 60 * 1000) < new Date()
    })
    .filter(function (user) {
